...
class Device:
    def __init__(self, name:str, tflops: float, memory_GB: float, bw_type:str, node_id: int, tp:int = 1):
        self.name = name
        self.tflops = tflops * tp  # 设备的计算能力，单位为 TFLOP/s
        self.memory_GB = memory_GB * tp  # 设备的显存大小，单位为 GB
        self.bw_type = bw_type  # 设备的带宽，单位为 Gbps
        self.bw = INTRA_BW[bw_type]  # 设备的带宽，单位为 Gbps
        self.node_id = node_id  # 设备所在的节点编号
        self.tp = tp  # 设备的张量并行度
...
class Model:
    ...
    def pp_active_mem_Gb(self) -> float:
        return self.microbatch_size * self.sequence_length * self.hidden_size * 16.0 / (10**9)  # Gbit
...
class Worker:
    ...
    def F_time_per_layer(self)-> float: # seconds
        ...
        if not self.exist_profiling():
            ...
            return self.get_profiling("forward_time_per_layer") / 1000000.0  # us转为秒
    ...
    def pp_activation_transfer_time(self, aim_worker: 'Worker')-> float:
        if self.device.node_id == aim_worker.device.node_id:
            bw = min(self.device.bw, aim_worker.device.bw)
        else:
            bw = min(INTER_BW, self.device.bw, aim_worker.device.bw)
            
        return self.model.pp_active_mem_Gb() / bw
...
def get_model(model_name: str) -> Model:
    ...
    elif model_name == "gpt3_175b-tune":
        res = Model(
            name="gpt3_175b-tune",   # GPT-3 Davinci
            layer_num=48,
            batch_size=8,
            microbatch_size=1,
            sequence_length=2048,
    ...
    elif model_name == "gpt3_175b":
        res = Model(
            name="gpt3_175b",   # GPT-3 Davinci
            layer_num=96,
            batch_size=16,
            microbatch_size=1,
            sequence_length=2048,
...
def get_device(device_name: Dict) -> Device:
    # 根据设备名称返回对应的Device对象
    node_id = device_name['node_id']
    ...
    elif device_name == "RTX5090-32GB-TP8":
        return Device(
            name="5090",
            tflops=419,  # FP16 TFLOPS
            memory_GB=32.0,
